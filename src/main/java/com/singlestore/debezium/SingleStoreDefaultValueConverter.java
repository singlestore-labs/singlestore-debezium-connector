...
public class SingleStoreDefaultValueConverter implements DefaultValueConverter {
  ...
  public Optional<Object> parseDefaultValue(Column column, String defaultValueExpression) {
    ...
    if (convertedDefaultValue instanceof Struct) {
      ...
      LOGGER.warn("Struct can't be used as default value for column '{}', will use null instead.",
          column.name());
      return Optional.empty();
    }
  ...
  /**
   * Converts a string object for an object type of {@link LocalDateTime}, set epoch timestamp
   * 1970-01-01 00:00:00 in case if default * value is failed to parse and column is not optional.
   *
   * @param column the column definition describing the {@code data} value; never null
  ...
  private Object convertToLocalDateTime(Column column, String value) {
    DateTimeFormatter formatter =
        column.length() > 25 ? DATE_TIME_FORMATTER_WITH_MS : DATE_TIME_FORMATTER;
    try {
