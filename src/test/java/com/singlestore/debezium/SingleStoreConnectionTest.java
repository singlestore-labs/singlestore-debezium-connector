...
public class SingleStoreConnectionTest {
  ...
  public void testJdbcParameters() {
    SingleStoreConnection connection = createConnectionWithParams(
        Map.of(SingleStoreConnectorConfig.DRIVER_PARAMETERS,
            "param1=value1;param2=value2;param3=value3"));
    assertEquals(Map.of("param1", "value1", "param2", "value2", "param3", "value3"),
        connection.connectionConfig().driverParameters());
  }

  @Test
  public void testSslDisabledParams() {
    SingleStoreConnection connection = createConnectionWithParams(
        Map.of(SingleStoreConnectorConfig.SSL_TRUSTSTORE, "trustStorePath",
            SingleStoreConnectorConfig.SSL_TRUSTSTORE_PASSWORD, "pass",
            SingleStoreConnectorConfig.SSL_KEYSTORE, "keyStorePath",
            SingleStoreConnectorConfig.SSL_KEYSTORE_PASSWORD, "pass"));
  ...
  public void testSslVerifyParams() {
    SingleStoreConnection connection = createConnectionWithParams(
        Map.of(SingleStoreConnectorConfig.SSL_MODE, "verify_ca",
            SingleStoreConnectorConfig.SSL_TRUSTSTORE, "trustStorePath",
            SingleStoreConnectorConfig.SSL_TRUSTSTORE_PASSWORD, "pass",
            SingleStoreConnectorConfig.SSL_KEYSTORE, "keyStorePath",
            SingleStoreConnectorConfig.SSL_KEYSTORE_PASSWORD, "pass"));
    ...
    assertEquals("file:trustStorePath",
        connection.connectionConfig().config().getString("trustStore"));
  }
  ...
  public void testObserveWithTableAndColumnFilter() throws SQLException {
    ...
    verify(connection).query(matches(
            "OBSERVE `debezium`.`table[12]`.`field1`,`debezium`.`table[21]`.`field1` FROM `debezium`.`table[12]`,`debezium`.`table[12]`"),
        any());
  }
  ...
  public void testObserveWithFilter() throws SQLException {
    ...
    verify(connection).query(matches(
            "OBSERVE `debezium`.`table[12]`.`field1`,`debezium`.`table[21]`.`field1` FROM `debezium`.`table[12]`,`debezium`.`table[12]` AS JSON BEGIN AT \\(1, 2, NULL, 4\\) WHERE `table1`.`filed1`=1"),
        any());
  }
  ...
  private SingleStoreConnection createConnectionWithParams(Map<Field, String> fieldMap) {
    ...
    return new SingleStoreConnection(
        new SingleStoreConnection.SingleStoreConnectionConfiguration(builder.build()));
  }
}
