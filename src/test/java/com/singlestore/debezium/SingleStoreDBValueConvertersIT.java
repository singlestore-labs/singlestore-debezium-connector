...
public class SingleStoreDBValueConvertersIT extends IntegrationTestBase {
    ...
    private void testTimeAndDateValues(TemporalPrecisionMode mode) {
        ...
        try (SingleStoreDBConnection conn = new SingleStoreDBConnection(defaultJdbcConnectionConfig())) {
            ...
            if (mode == TemporalPrecisionMode.ADAPTIVE || mode == TemporalPrecisionMode.ADAPTIVE_TIME_MICROSECONDS) {
                ...
                testColumn(converters, table, "dateColumn", java.sql.Date.valueOf(date), (int) date.atStartOfDay(ZoneId.of("UTC")).toEpochSecond() / 60 / 60 / 24); //epoch days
                testColumn(converters, table, "datetimeColumn", java.sql.Timestamp.valueOf(dateTime), Date.from(dateTime.atZone(ZoneId.of("UTC")).toInstant()).getTime()); // milliseconds
                testColumn(converters, table, "datetime6Column", java.sql.Timestamp.valueOf(dateTime6), Date.from(dateTime.atZone(ZoneId.of("UTC")).toInstant()).getTime() * 1_000 + 123456);  //microseconds
                testColumn(converters, table, "timestampColumn", java.sql.Timestamp.valueOf(dateTime), Date.from(dateTime.atZone(ZoneId.of("UTC")).toInstant()).getTime());
                testColumn(converters, table, "timestamp6Column", java.sql.Timestamp.valueOf(dateTime6), Date.from(dateTime.atZone(ZoneId.of("UTC")).toInstant()).getTime() * 1_000 + 123456);
            } else {
                ...
                testColumn(converters, table, "dateColumn", java.sql.Date.valueOf(date), new Date(date.atStartOfDay(ZoneId.of("UTC")).toEpochSecond() * 1_000)); //epoch seconds as date
                testColumn(converters, table, "datetimeColumn", java.sql.Timestamp.valueOf(dateTime), Date.from(dateTime.atZone(ZoneId.of("UTC")).toInstant())); // milliseconds
                testColumn(converters, table, "datetime6Column", java.sql.Timestamp.valueOf(dateTime6), new Date(Date.from(dateTime.atZone(ZoneId.of("UTC")).toInstant()).getTime() + 123));  //milliseconds
                testColumn(converters, table, "timestampColumn", java.sql.Timestamp.valueOf(dateTime), Date.from(dateTime.atZone(ZoneId.of("UTC")).toInstant()));
                testColumn(converters, table, "timestamp6Column", java.sql.Timestamp.valueOf(dateTime6), new Date(Date.from(dateTime.atZone(ZoneId.of("UTC")).toInstant()).getTime() + 123));
            }
        } catch (SQLException e) {
            Assert.fail(e.getMessage());
        }
    ...
    public void testStringValues() {
        try (SingleStoreDBConnection conn = new SingleStoreDBConnection(defaultJdbcConnectionConfig())) {
            ...
            Table table = tables.forTable(TEST_DATABASE, null, "allTypesTable");
            assertThat(table).isNotNull();
            testColumn(CONVERTERS, table, "jsonColumn", "{ \"key1\": \"val1\", \"key2\": {\"key3\":\"val3\"} }", "{ \"key1\": \"val1\", \"key2\": {\"key3\":\"val3\"} }");
    ...
    private void testBinaryMode(CommonConnectorConfig.BinaryHandlingMode mode) {
        ...
        try (SingleStoreDBConnection conn = new SingleStoreDBConnection(defaultJdbcConnectionConfig())) {
            ...
            String base64value = new String(Base64.getEncoder().encode(value.getBytes(StandardCharsets.UTF_8)));
            String base64urlValue = new String(Base64.getUrlEncoder().encode(value.getBytes(StandardCharsets.UTF_8)));
            if (mode == CommonConnectorConfig.BinaryHandlingMode.BYTES) {
                testColumn(converters, table, "varbinaryColumn", value, ByteBuffer.wrap(value.getBytes(StandardCharsets.UTF_8)));
                testColumn(converters, table, "blobColumn", value, ByteBuffer.wrap(value.getBytes(StandardCharsets.UTF_8)));
                ...
                testColumn(converters, table, "varbinaryColumn", value, HexConverter.convertToHexString(value.getBytes(StandardCharsets.UTF_8)));
                testColumn(converters, table, "blobColumn", value, HexConverter.convertToHexString(value.getBytes(StandardCharsets.UTF_8)));
            }
        } catch (SQLException e) {
            Assert.fail(e.getMessage());
        }
